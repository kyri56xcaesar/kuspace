...
func (m *PlainHandler) Useradd(user ut.User) (int, int, error) {
	log.Printf("Adding user %q ...", user.Username)
	...
	// get uuid
	uuid := nextUid()
	fmt.Fprintf(file, ENTRY_MPASSWD_FORMAT, user.Username, PLACEHOLDER_PASS, uuid, uuid, user.Info, user.Home, user.Shell)
...
// Unused
func checkIfUserExists(u ut.User) error {
	log.Printf("Checking if name:%s, already exists...", u.Username)
	var users []string
...
func exists(user *ut.User) error {
	log.Printf("checking if %q exists...", user.Username)
	file, err := os.Open(MINIOTH_PASSWD)
	if err != nil {
		log.Printf("error opening file: %v", err)
		return err
	...
	defer file.Close()
	res, line, err := search(user.Username, file)
	if err == nil && line != -1 {
...
func getGroups(username string, file *os.File) ([]ut.Group, error) {
	if username == "" || file == nil {
