...
and machine learning pipelines. Examples include Apache Airflow~\cite{airflow}, Apache Spark~\cite{spark}, and Hadoop~\cite{luigi}.
...
Apache Airflow is a platform to programmatically author, schedule, and monitor workflows. It allows
users to define workflows as directed acyclic graphs (DAGs) using Python code. Airflow is designed for complex data pipelines,
providing features like task dependencies, retries, and scheduling. However, it is primarily focused on
orchestrating workflows rather than providing a user-friendly interface for individual users to run ad-hoc jobs.
...
It is designed to scale up from a single server to thousands of machines, each offering local computation
and storage. Hadoop is often used for batch processing of large data sets, but it requires significant
infrastructure and is not user-friendly for individual users who want to run small, isolated jobs.

\subsection{Apache Spark}
Apache Spark is a unified analytics engine for large-scale data processing, known for its speed and ease
of use. It provides APIs in Java, Scala, Python, and R, and supports SQL queries, streaming data, machine learning, and graph processing.
Spark is designed for distributed data processing and is often used in big data environments. However, it
requires significant infrastructure setup and is not tailored for individual users who want to run
ad-hoc analytical jobs in a multi-user environment.
