...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.3\textwidth]{Images/kuspace-logo.png}
...
\section{Fslite: Storage Abstraction}
...
\subsection{Integration}
Both the Frontapp and the Wss are integrated in the system as Kubernetes Deployments remain stateless. There is also ephemeral logging, 
...
executed. It also deploys each Microservice itself, tightening overall security and exposes only the desired service to the end user. In 
this case everything should be used and accessed via Frontapp.

Additionally, Kubernetes' Persistent Volume Claims (PVCs) or object storage access is used in alignment with StatefulSets to persist data. 
